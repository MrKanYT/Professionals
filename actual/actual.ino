...
int _gtv30 = 300; //Частота моргания светодиодов. Если сделать большую - горит долго ;)
...
//Комментарии:Параметр таймера регулирует частоту опроса датчиков и передачу телеметрии.
    ...
    if (_tim1O) 
    {
        _gtv18 = (_poligon(((analogRead (12))), _Poligon_InArray_1, _Poligon_OutArray_1, 13));
    }
    if (_tim1O) 
    {
...
//Наименование:Расшифровка принятого из порта
    if (_gtv10 == 1) 
    {
        ...
        if (_FSFS7CO) 
        {
            _gtv30 = (constrain((_convertStringToNamberOutput_10), (10), (30000)));
        }
...
//Наименование:Задание скорости и ускорения при движении по энкодеру
    ...
    if(EnOcto)
    {
        ...
        Error = int(octoliner.trackLine(lineData)*10);
    }
    _gtv32 = Error;
    //Плата:12
